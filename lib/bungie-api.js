// lib/bungie-api.js - Correctly Mapped Tier Types
// Based on actual observed data: everything was shifted by one level

// Corrected Tier type constants
export const TIER_TYPES = {
  UNKNOWN: 0,
  CURRENCY: 1,
  COMMON: 2,      // White items
  UNCOMMON: 3,    // Green items (we were calling this "Rare")
  RARE: 4,        // Blue items (we were calling this "Legendary")
  LEGENDARY: 5,   // Purple items (we were calling this "Exotic")
  EXOTIC: 6       // Yellow items (we were calling this "Unknown")
};

// Corrected helper function with proper Destiny rarity names
export const getTierTypeName = (tierType) => {
  switch(tierType) {
    case 0:
    case TIER_TYPES.UNKNOWN: 
      return 'Unknown';
    case 1:
    case TIER_TYPES.CURRENCY: 
      return 'Currency';
    case 2:
    case TIER_TYPES.COMMON: 
      return 'Common';         // White items
    case 3:
    case TIER_TYPES.UNCOMMON: 
      return 'Uncommon';       // Green items
    case 4:
    case TIER_TYPES.RARE: 
      return 'Rare';           // Blue items
    case 5:
    case TIER_TYPES.LEGENDARY: 
      return 'Legendary';      // Purple items
    case 6:
    case TIER_TYPES.EXOTIC: 
      return 'Exotic';         // Yellow items
    default: 
      return `Tier ${tierType}`;
  }
};

// Updated helper functions with correct values
export const isExotic = (item) => {
  return item.inventory?.tierType === 6; // 6 is exotic
};

export const isLegendary = (item) => {
  return item.inventory?.tierType === 5; // 5 is legendary
};

export const isRare = (item) => {
  return item.inventory?.tierType === 4; // 4 is rare (blue)
};

export const isUncommon = (item) => {
  return item.inventory?.tierType === 3; // 3 is uncommon (green)
};

export const isCommon = (item) => {
  return item.inventory?.tierType === 2; // 2 is common (white)
};

// Keep all the existing constants and functions below...
